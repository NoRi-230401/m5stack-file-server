...
bool SPIFFS_notFoundV11(AsyncWebServerRequest *request);
...
extern bool SD_notFoundV20(AsyncWebServerRequest *request);
// -------------------------------------------------------
...
void SPIFFS_setupServerV11()
{
  ...
  server.on("/handleupload", HTTP_POST, [](AsyncWebServerRequest *request) {}, [](AsyncWebServerRequest *request, const String &filename, size_t index, uint8_t *data, size_t len, bool final)
...
void notFound(AsyncWebServerRequest *request)
{
  ...
  if (SD_notFoundV20(request))
    return;

  if (SPIFFS_notFoundV11(request))
    return;
...
{ // Process selected file types
  Serial.println("SPIFFS_notFundV11 func ...");
